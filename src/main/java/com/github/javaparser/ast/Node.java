 ...
 */
public abstract class Node  extends NodeWithRange<Node> implements Cloneable, HasParentNode<Node>, Visitable,NodeWithTokenRange<Node> {
    ...
    public static Comparator<NodeWithRange<?>> NODE_BY_BEGIN_POSITION =new Comparator<NodeWithRange<?>>() {
		
		@Override
		public int compare(NodeWithRange<?> a, NodeWithRange<?> b) {
    ...
    public Comment getCommentOptional() {
        return comment;
    }
    ...
    public Node setTokenRange(TokenRange tokenRange) {
        ...
        if (tokenRange == null || !(tokenRange.getBegin().getRange()!=null && tokenRange.getBegin().getRange()!=null)) {
            range = null;
        } else {
    ...
    @Override
    public Node getOptionalParentNode() {
        return parentNode;
    }
    ...
    public void tryAddImportToParentCompilationUnit(Class<?> clazz) {
        //getAncestorOfType(CompilationUnit.class).ifPresent(p -> p.addImport(clazz));
    	 CompilationUnit  compilationUnit =getAncestorOfType(CompilationUnit.class);
    ...
    public void removeForced() {
        if (!remove()) {
        	/*getOptionalParentNode().ifPresent(Node::remove);*/
    ...
    public Node findRootNode() {
        Node n = this;
        while (n.getOptionalParentNode()!=null) {
            n = n.getOptionalParentNode();
        }
        return n;
    }
    ...
    private Iterable<Node> treeIterable(TreeTraversal traversal) {
    	...
    	final TreeTraversal t=traversal;
     ...
     */
    /*public Node stream(TreeTraversal traversal) {
       // return StreamSupport.stream(Spliterators.spliteratorUnknownSize(treeIterator(traversal), NONNULL | DISTINCT), false);
    	//return StreamSupport.stream(, NONNULL | DISTINCT), false);
    	return Spliterators.spliteratorUnknownSize(treeIterator(traversal);
    }
    ...
    *//**
    ...
    public <T extends Node> void walk(final Class<T> nodeType, final Consumer<T> consumer) {
       /* walk(TreeTraversal.PREORDER, node -> {
    	...
    	Consumer<Node> con=new Consumer<Node>() {
			@Override
			public void accept(Node t) {
				if (nodeType.isAssignableFrom(t.getClass())) {
	                consumer.accept(nodeType.cast(t));
	            }
			}
    		
		};
		
		walk(TreeTraversal.PREORDER,con);
    ...
    public <T extends Node> List<T> findAll(Class<T> nodeType, final Predicate<T> predicate) {
      ...
      /*  walk(nodeType, n -> {
        ...
        Consumer<T> con=new Consumer<T>() {
			...
			@Override
			public void accept(T t) {
				 if (predicate.test(t)) found.add(t);
			}
		};
     ...
     */
    public <T> T findFirst(TreeTraversal traversal, Function<Node, T> consumer) {
        for (Node node : treeIterable(traversal)) {
            final T result = consumer.apply(node);
            if (result!=null) {
                return result;
            }
        }
     ...
     */
    public <N extends Node> N findFirst(final Class<N> nodeType) {
       /* return findFirst(TreeTraversal.PREORDER, node -> {
    	...
    	Function<Node, N> f=new Function<Node, N>() {

			@Override
			public  N apply(Node t) {
				if (nodeType.isAssignableFrom(t.getClass())) {
	                return nodeType.cast(t);
	            }
	            return null;
			}
		};
		return findFirst(TreeTraversal.PREORDER,f);
     ...
     * found.
     */
    public <N extends Node> N findFirst(final Class<N> nodeType, final Predicate<N> predicate) {
       /* return findFirst(TreeTraversal.PREORDER, node -> {
            if (nodeType.isAssignableFrom(node.getClass())) {
     ...
     */
    public <N extends Node> N findParent(Class<N> nodeType) {
        Node n = this;
        while (n.getOptionalParentNode()!=null) {
            n = n.getOptionalParentNode();
            if (nodeType.isAssignableFrom(n.getClass())) {
                return nodeType.cast(n);
            }
        }
        return null;
    }
    ...
    public static class ParentsVisitor implements Iterator<Node> {
        ...
        @Override
        public boolean hasNext() {
        	if(node.getOptionalParentNode()!=null) return true;
        	else return false;
//           / return node.getOptionalParentNode().isPresent();
        ...
        public Node next() {
           ...
           /* node = node.getOptionalParentNode().orElse(null);*/
            return node;
        }
